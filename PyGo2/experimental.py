...
def quiescence(board, alpha, beta, color, nodes, depth):
    ...
    for move in captures:
        board.push(move)
        nodes += 1
        score, nodes = quiescence(board, -beta, -alpha, -color, nodes, depth - 1)
        score = -score
        board.pop()
...
def negamax(board, depth, alpha, beta, color, nodes):
    ...
    if depth == 0:
        score, nodes = quiescence(board, alpha, beta, color, nodes, MAX_QUIESCENCE_DEPTH)
...
def play_game():
    board = chess.Board(fen = "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - ")
    eval = 0
    nodes = 0
    elapsed_time = 0
